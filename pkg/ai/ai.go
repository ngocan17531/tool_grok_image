...
func Bulk(parentCtx context.Context, cli Client, prompts []string, skip []int, variationEnabled, upscaleEnabled bool, concurrency int, wait time.Duration) (<-chan *Image, <-chan error) {
		...
		go func() {
			...
			for k, e := range entries {
				...
				if err != nil {
					...
					// Skip this prompt but notify progress
					out <- &Image{
						Prompt:      e.prompt,
						PromptIndex: e.index,
						IsLast:      true,
				...
				for i := range preview.ImageIDs {
					if upscaleEnabled {
						...
						if err != nil {
							...
							// Skip the rest of this prompt grid but ensure progress moves
							if i == len(preview.ImageIDs)-1 && !variationEnabled {
								out <- &Image{
									Prompt:      e.prompt,
									PromptIndex: e.index,
									IsLast:      true,
					...
					for j := range variationPreview.ImageIDs {
						...
						if err != nil {
							...
							if i == len(preview.ImageIDs)-1 && j == len(variationPreview.ImageIDs)-1 {
								out <- &Image{
									Prompt:      e.prompt,
									PromptIndex: e.index,
									IsLast:      true,
