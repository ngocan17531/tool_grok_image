...
func StringToSpec(ja3 string, userAgent string) (*utls.ClientHelloSpec, error) {
	...
	for _, c := range curves {
		...
		curveID := utls.CurveID(cid)
		// Go's crypto/tls only supports these curves for TLS 1.3
		if curveID == utls.CurveP256 || curveID == utls.CurveP384 || curveID == utls.CurveP521 || curveID == utls.X25519 {
			targetCurves = append(targetCurves, curveID)
		}
	}
