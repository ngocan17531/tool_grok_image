...
import (
	...
	"BulkAI/pkg/ai"
	"BulkAI/pkg/ai/bluewillow"
	"BulkAI/pkg/ai/midjourney"
	"BulkAI/pkg/discord"
	"BulkAI/pkg/http"
	"BulkAI/pkg/img"
	"gopkg.in/yaml.v2"
...
func Generate(ctx context.Context, cfg *Config, opts ...Option) error {
	...
	imageChan, errChan := ai.Bulk(ctx, cli, prompts, album.Finished, cfg.Variation, cfg.Upscale, cfg.Concurrency, cfg.Wait)
	var exit bool
	for !exit {
		var status string
		select {
